...
/**
 * Multipreview annotations for the Cine design system.
 *
 * Annotating a preview function with one of these renders it once per variant,
 * so a single `@CineThemePreview fun` proves a component in both themes without
 * duplicated preview code. Combine them freely — annotations multiply, so
 * `@CineThemePreview @CineFontScalePreview` yields 2 × 4 renders.
 *
 * Background colours below are the raw `CineColors` grounds (Night900 / the
...
annotation class CineThemePreview
...
annotation class CineFontScalePreview

/**
 * The three window-size classes the app must support (§5.3). Used on screens
 * and on `CineAppScaffold`, which switches between bottom bar and rail at 600dp.
 */
...
annotation class CineDevicePreview
...
annotation class CineRtlPreview
